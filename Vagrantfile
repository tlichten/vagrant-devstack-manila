...
Vagrant.configure(2) do |config|
  ...
  config.vm.define "devstack" do |devstack|
  	devstack.vm.box = "chef/centos-7.0"
	...
	devstack.vm.network "private_network", ip: DEVSTACK_HOST_IP
	devstack.vm.network "private_network", ip: DEVSTACK_MGMT_IP
	devstack.vm.provision :shell, :path => "vagrant.sh"
	if Vagrant.has_plugin?("vagrant-cachier")
	  devstack.cache.scope = :box
	end
  end
end
